...
let noFireRate; //Number of consecutive times no fire rate bonus appears
...
class SoundPool {
    ...
    play() {
      ...
      for (let sound of this.pool) {
        if (sound.paused) {
            if (Math.random()>0.5 || count>5){
                sound.currentTime = 0;
                sound.play();
                console.log("Successfully played sound effect: "+sound.src);
                return;
            }else{
                count++;
            }
      ...
      console.log("All sound effects are playing");
    }
}
...
function drawUI() {
    ...
    ctx.lineWidth = 3;
    ctx.strokeText(`Bullet Limit: ${(Maxbullets[player.bulletType-1])}`, 10, 760);
ctx.fillText(`Bullet Limit: ${(Maxbullets[player.bulletType-1])}`, 10, 760);
    ctx.textAlign = 'right';
    ctx.strokeText(`Damage: ${player.damage.toFixed(1)}`, canvas.width-10, 760);
ctx.fillText(`Damage: ${player.damage.toFixed(1)}`, canvas.width-10, 760);
    if (defaults){
        ctx.fillStyle = "#f00"
        ctx.strokeText(`Default Bonus Active`, canvas.width-10, 740);
ctx.fillText(`Default Bonus Active`, canvas.width-10, 740);
    }
    if (stops || gameOver){
        ctx.textAlign = 'left';
...
function gameLoop() {
    ...
    if (lastSec+1/timeScale < currentTime/1000){
        ...
        if (player.bulletType==6 && !comp){
            addAchievement("Congratulations on Clear #"+(achievement[4]+1)+"!",4,bType6);
            comp = true;
        }
        AchievementCount();
...
let rtypes = 'Bullet';
        ...
        if ((firstV<=0 || ltypes !== 'Fire Rate') && (secV<=0 || mtypes !== 'Fire Rate') && (thirdV<=0 || rtypes !== 'Fire Rate')){
            noFireRate++;
        }else{
            noFireRate = 0;
        }

        if (ltypes == 'Damage') firstV *=2;
if (mtypes == 'Damage') secV *=2;
if (rtypes == 'Damage') thirdV *=2;
...
function applyUpgrade(barrier) {
    ...
    if (player.damage<0){
        addAchievement("Village's Best Healer!",0,heart);
    }
    if (player.fireRate>=2000){
        addAchievement("Snail",1,bTypes[player.bulletType-1]);
    }
    if (player.bulletCount===0){
        addAchievement("Lay Down Your Weapon",2,bTypes[player.bulletType-1]);
    }
    if (oldBcount === 0 && player.bulletCount>0){
        addAchievement("Last-Minute Save!",3,bTypes[player.bulletType-1]);
    }
    if (0===1){
        addAchievement("Math is Broken!",-1,heart);
    }
}
...
function displayAchievement(){
    ...
    ctx.textAlign = 'left';
    ctx.fillStyle = '#ff0';
    ctx.fillText("Achievement Unlocked! ",canvas.width-270,dA.height-40);
    ctx.font = '20px Arial';
    ctx.fillStyle = '#fff';
